...
# to sweep.sh. Needs: hcloud CLI with a context selected, ~/.ssh/id_ed25519(.pub), rsync.
#
...
if [ "$IMAGE" = auto ]; then
  IMAGE=$(hcloud image list --type snapshot -l lab-image=1 -o noheader -o columns=id,created | sort -k2 | tail -1 | awk '{print $1}')
  [ -n "$IMAGE" ] || IMAGE=none
fi
...
for ip in "${ips[@]}"; do
  ...
  rsync -az -e "$RSYNC_SSH" root@"$ip":/root/lab-out/sweep.log "$DEST"/sweep.$i.log
  i=$((i + 1))
done
