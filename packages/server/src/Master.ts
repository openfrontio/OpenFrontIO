import { getServerConfigFromServer } from "@openfrontio/core/src/configuration/Config";
import {
  Difficulty,
  GameMapType,
  GameType,
...
import type { GameInfo } from "@openfrontio/core/src/Schemas";
...
async function fetchLobbies(): Promise<void> {
  ...
  lobbyInfos.forEach((lobby) => {
    if (lobby.msUntilStart <= 250) {
      publicLobbyIDs.delete(lobby.gameID);
    }
  });
