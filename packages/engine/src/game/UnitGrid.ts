...
export class UnitGrid {
  private grid: Map<UnitType, Set<UnitLike>>[][];
    ...
    this.grid = Array(Math.ceil(gm.height() / this.cellSize))
      ...
      .map(() =>
        Array(Math.ceil(gm.width() / this.cellSize))
          .fill(null)
          .map(() => new Map<UnitType, Set<UnitLike>>()),
      );
  }
  ...
  removeUnit(unit: UnitLike) {
    const tile = unit.tile();
    this.removeUnitByTile(unit, tile);
  }

  removeUnitByTile(unit: UnitLike, tile: TileRef) {
    const [gridX, gridY] = this.getGridCoords(this.gm.x(tile), this.gm.y(tile));
  ...
  nearbyUnits(
    ...
    const nearby: Array<{ unit: UnitLike; distSquared: number }> = [];
    const gm = this.gm;
    const x = gm.x(tile);
    const y = gm.y(tile);
