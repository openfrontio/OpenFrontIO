...
const RELATIVE_PATH_TO_MAPS = path.resolve(
  __dirname,
  "..",
  "..",
  "core",
...
async function loadTerrainMap(mapName: string): Promise<void> {
  const imagePath = path.resolve(RELATIVE_PATH_TO_MAPS, mapName + ".png");

  const bunStream = Bun.file(imagePath).stream();
  const img = await decodePNGFromStream(Readable.from(bunStream));
  ...
  const outputPath = path.join(RELATIVE_PATH_TO_MAPS, `${mapName}.bin`);
...
if (!Bun.isMainThread) {
  self.onmessage = async (event: MessageEvent) => {
    if (typeof event.data === "string") {
      const mapName = event.data;
      console.log(`Generating map for ${mapName}`);
      try {
        await loadTerrainMap(mapName);
        console.log(`Map for ${mapName} generated successfully`);
        postMessage(true);
      } catch (err) {
        console.error(`Could not generate map for ${mapName}`, err);
      }
    }
  };
}
