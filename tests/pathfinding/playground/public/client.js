...
function initializeControls() {
    ...
    document.getElementById('welcomeMapSelect').addEventListener('change', (e) => {
    ...
    document.getElementById('requestPfMini').addEventListener('click', () => {
        if (state.startPoint && state.endPoint && !state.pfMiniPath && !state.isPfMiniLoading) {
    ...
    ['showInitialPath', 'showUsedGateways', 'showColoredMap', 'showGateways', 'showSectorGrid', 'showEdges'].forEach(id => {
...
function initializeDragControls() {
    ...
    wrapper.addEventListener('mousemove', (e) => {
        ...
        if (draggingPoint) {
            ...
            const showGateways = document.getElementById('showGateways').dataset.active === 'true';
            const showUsedGateways = document.getElementById('showUsedGateways').dataset.active === 'true';

            if ((showGateways || showUsedGateways) && state.graphDebug && state.graphDebug.allGateways) {
                // Filter gateways based on what's visible
                let gatewaysToCheck = state.graphDebug.allGateways;
                if (showUsedGateways && !showGateways && state.debugInfo && state.debugInfo.gatewayWaypoints) {
                ...
                const foundGateway = findGatewayAtPosition(canvasX, canvasY, gatewaysToCheck);
...
function handleMapClick(e) {
    if (!state.currentMap || state.isMapLoading || state.isNavMeshLoading || state.isPfMiniLoading) return;
...
function updateTimingsPanel(result) {
    ...
    const findGatewaysValueEl = document.getElementById('timingFindGatewaysValue');
    ...
    if (result.pfMini && result.pfMini.time > 0) {
        ...
        document.getElementById('pfMiniTiles').textContent = `- ${result.pfMini.length} tiles`;
...
function renderMapBackground(scale) {
    ...
    const imageData = mapCtx.createImageData(state.mapWidth * scale, state.mapHeight * scale);
    const data = imageData.data;

    // Check if colored map is enabled
    const showColored = document.getElementById('showColoredMap').dataset.active === 'true';

    let waterR, waterG, waterB, landR, landG, landB;

    if (showColored) {
        // Colored: Water = #2a5c8a (darker blue), Land = #a1bb75
        waterR = 42; waterG = 92; waterB = 138;
        landR = 161; landG = 187; landB = 117;
    } else {
        // Grayscale: Water = #3c3c3c (darker gray), Land = #777777 (slightly darker)
        waterR = 60; waterG = 60; waterB = 60;
        landR = 119; landG = 119; landB = 119;
    }
...
function renderOverlay(scale) {
    ...
    const showSectorGrid = document.getElementById('showSectorGrid').dataset.active === 'true';
    const showEdges = document.getElementById('showEdges').dataset.active === 'true';
    const showGateways = document.getElementById('showGateways').dataset.active === 'true';
    ...
    if (showEdges && state.graphDebug.edges) {
        ...
        for (const edge of state.graphDebug.edges) {
            overlayCtx.beginPath();
            overlayCtx.moveTo((edge.from[0] + 0.5) * scale, (edge.from[1] + 0.5) * scale);
    ...
    if (showGateways && state.graphDebug.allGateways) {
        ...
        for (const gw of state.graphDebug.allGateways) {
            overlayCtx.beginPath();
            overlayCtx.arc((gw.x * 2 + 0.5) * scale, (gw.y * 2 + 0.5) * scale, gatewayRadius, 0, Math.PI * 2);
            overlayCtx.fill();
        }
    }
}
...
function renderInteractive() {
    ...
    // Check what to show
    const showUsedGateways = document.getElementById('showUsedGateways').dataset.active === 'true';
    const showInitialPath = document.getElementById('showInitialPath').dataset.active === 'true';
    const showEdges = document.getElementById('showEdges').dataset.active === 'true';
    const showGateways = document.getElementById('showGateways').dataset.active === 'true';

    // Draw highlighted edges for hovered gateway only
    if (hoveredGateway && showEdges && state.graphDebug && state.graphDebug.edges) {
    ...
    if (hoveredGateway && showGateways && state.graphDebug && state.graphDebug.allGateways) {
        ...
        if (state.graphDebug.edges) {
            ...
            connectedEdges.forEach(edge => {
                if (edge.fromId !== hoveredGateway.id) connectedGatewayIds.add(edge.fromId);
        ...
        for (const gwId of connectedGatewayIds) {
            ...
            if (gw) {
                ...
                interactiveCtx.beginPath();
                interactiveCtx.arc(screen.x, screen.y, Math.max(3, zoomLevel * 2), 0, Math.PI * 2);
                interactiveCtx.fill();
                interactiveCtx.stroke();
            }
        }
        ...
        interactiveCtx.beginPath();
        interactiveCtx.arc(screen.x, screen.y, Math.max(4, zoomLevel * 2.5), 0, Math.PI * 2);
        interactiveCtx.fill();
        interactiveCtx.stroke();
    }

    // Draw initial path (unsmoothed)
    if (showInitialPath && state.debugInfo && state.debugInfo.initialPath && state.debugInfo.initialPath.length > 0) {
...
function findGatewayAtPosition(canvasX, canvasY, gatewaysToCheck = null) {
    const gateways = gatewaysToCheck || (state.graphDebug && state.graphDebug.allGateways);
    if (!gateways) {
        return null;
    }
...
function showGatewayTooltip(gateway, mouseX, mouseY) {
    ...
    const selfLoops = connectedEdges.filter(
        e => e.fromId === e.toId
    );
    ...
    if (connectedEdges.length > 0) {
        ...
        const outgoing = connectedEdges.filter(e => e.fromId === gateway.id && e.toId !== gateway.id);
        const incoming = connectedEdges.filter(e => e.toId === gateway.id && e.fromId !== gateway.id);
