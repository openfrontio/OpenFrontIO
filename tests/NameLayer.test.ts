...
describe("NameLayer", () => {
  describe("computeAllianceClipPath", () => {
    ...
    beforeEach(() => {
      // Create a minimal NameLayer instance for testing
      // We don't need full initialization since we're only testing a pure function
      nameLayer = Object.create(NameLayer.prototype);
    });

    test("returns full visibility (20% top cut) when alliance time is at 100%", () => {
      const result = nameLayer["computeAllianceClipPath"](1.0);
    ...
    test("returns maximum cut (82.40% top cut) when alliance time is at 0%", () => {
      const result = nameLayer["computeAllianceClipPath"](0.0);
    ...
    test("returns 51.20% top cut when alliance time is at 50%", () => {
      const result = nameLayer["computeAllianceClipPath"](0.5);
    ...
    test("returns 27.80% top cut when alliance time is at 87.5%", () => {
      const result = nameLayer["computeAllianceClipPath"](0.875);
    ...
    test("returns 74.60% top cut when alliance time is at 12.5%", () => {
      const result = nameLayer["computeAllianceClipPath"](0.125);
