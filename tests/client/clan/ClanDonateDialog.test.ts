...
describe("clan-donate-dialog", () => {
  ...
  it("shows the player's balance for each currency, soft selected by default", () => {
    ...
    // The dialog formats balances with toLocaleString(), so the expected
    // separators depend on the host locale. Build the expectation the same
    // way rather than hardcoding en-US output.
    expect(currencyButton("soft").textContent).toContain(
      `clan_modal.donate_your_balance:${JSON.stringify({
        balance: (1000).toLocaleString(),
      })}`,
    );
    expect(currencyButton("hard").textContent).toContain(
      `clan_modal.donate_your_balance:${JSON.stringify({
        balance: (25).toLocaleString(),
      })}`,
    );
  });
