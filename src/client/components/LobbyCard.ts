...
export function lobbyCard({
  ...
  // The lock and the expand control share the card's bottom-right corner; the
  // text has to clear whichever of them is drawn.
  const corners = (trustedOnly ? 1 : 0) + (onExpand ? 1 : 0);
  const cornerPad =
    corners === 2 ? "pr-[4.5rem]" : corners === 1 ? "pr-10" : "";
  ...
  return html`
    <div class="relative w-full ${heightClass}">
      ...
      >
        ...
        <div
          class="absolute bottom-0 left-0 right-0 flex flex-col px-3 py-2 bg-black/55 backdrop-blur-sm rounded-b-2xl ${cornerPad}"
          style="overflow: visible;"
        >
          ${trustedOnly
            ? trustLockIcon(viewerTrusted, onExpand !== undefined)
            : null}
          <span
...
// Bottom-right corner of the card, shifted left of the expand control when
// there is one. Red closed lock: the viewer can't join this trusted-only
// lobby. Green open lock: they can. Heroicons mini lock-closed / lock-open.
function trustLockIcon(
  ...
  return html`<span
    class="absolute bottom-2 ${shifted
      ? "right-11"
      : "right-2"} flex items-center bg-black/70 backdrop-blur-sm px-1.5 py-1 rounded ${viewerTrusted
      ? "text-green-400"
      : "text-red-400"}"
    title=${label}
    aria-label=${label}
