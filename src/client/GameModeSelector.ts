...
export class GameModeSelector extends LitElement {
  ...
  private renderOptionalLobbyCard(
    lobby: PublicGameInfo | undefined,
    heightClass: string,
    isSpecial: boolean = false,
  ) {
    if (!lobby) return nothing;
    return isSpecial
      ? this.renderSpecialLobbyCard(lobby, heightClass)
      : this.renderLobbyCard(lobby, this.getLobbyTitle(lobby), heightClass);
  }
  ...
  private renderActionCards(actions: ReadonlyArray<[string, () => void]>) {
    return actions.map(([translationKey, onClick]) =>
      this.renderSmallActionCard(translateText(translationKey), onClick),
    );
  }
