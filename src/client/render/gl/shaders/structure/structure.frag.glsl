...
// Atlas indices: 0=City, 1=Port, 2=Factory, 3=DefensePost, 4=SAM, 5=Silo, 6=Research
float shapeSDF(vec2 p, float R) {
  ...
  if (vAtlasIdx < 5.5)
    return sdPolygon(p, R, 3.0, PI * 0.5);    // Missile Silo: triangle
  return sdPolygon(p, R, 4.0, PI * 0.25);     // Research Facility: diamond
}
