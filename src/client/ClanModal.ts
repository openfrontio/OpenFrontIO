...
export class ClanModal extends BaseModal {
    ...
    if (result !== true) {
      window.dispatchEvent(
        new CustomEvent("show-message", {
          detail: {
            message: translateText(result.error),
            color: "red",
            duration: 3500,
          },
        }),
                  ...
                  >
                    ...
                    ${(() => {
                      ...
                      return html`
                        ...
                        ${this.renderMemberPagination(this.membersTotal)}
                      `;
                    })()}
                  </div>
                `
  ...
  private async handleDisband() {
    ...
    invalidateUserMe();
    this.myClanRoles.delete(this.selectedClan.tag);
    this.myClans = this.myClans.filter((c) => c.tag !== this.selectedClan!.tag);
    this.selectedClan = null;
    this.myRole = null;
    this.view = "list";
    this.loadMyClans();
    ...
    return html`
      ...
      >
        ...
        <div class="flex items-center gap-2">
          <button
            @click=${() =>
              this.loadMemberPage(Math.max(1, this.memberPage - 1))}
          ...
          <button
            @click=${() =>
              this.loadMemberPage(Math.min(totalPages, this.memberPage + 1))}
